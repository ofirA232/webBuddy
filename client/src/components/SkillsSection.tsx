...
const SkillsSection = () => {
  ...
  // Reads "01 / 05" (current / total) regardless of page direction.
  const Counter = ({ className = "" }: { className?: string }) => (
    <div dir="ltr" aria-live="polite" className={`flex items-baseline font-bold tabular-nums ${className}`}>
      ...
      <span className="text-[#666666]">{pad(slides.length)}</span>
    </div>
  );
  ...
  const Arrows = ({ className = "" }: { className?: string }) => (
    <div className={`flex gap-3 ${className}`}>
      <button
        type="button"
        onClick={prevSlide}
        className="w-10 h-10 sm:w-12 sm:h-12 border border-[#333] bg-transparent text-white hover:bg-[#111] transition-colors flex items-center justify-center rounded-md"
        ...
        <div className="flex flex-col md:flex-row md:items-center md:gap-6">
          ...
          <div className="w-full md:w-2/3 order-1 md:order-2 relative">
            ...
            <div className="flex flex-col items-center gap-4 mt-4 sm:mt-6 md:hidden">
              <Counter className="text-2xl sm:text-3xl" />
              <Arrows />
            </div>
          </div>

          {/* Text column (visually right on desktop) */}
          <div className="w-full md:w-1/3 md:text-right text-center flex flex-col justify-between order-2 md:order-1 mt-6 md:mt-0">
            <div className="flex flex-col md:items-end items-center">
              <div className="w-full mb-8 md:mb-12">
                <h3 className="text-2xl md:text-3xl text-white font-bold mb-4">{slide.title}</h3>
                <p className="text-[1rem] md:text-[1.125rem] text-gray-300 leading-relaxed">{slide.description}</p>
              </div>

              <div className="w-full">
                <div className="grid grid-cols-2 gap-x-4 gap-y-4 md:justify-items-start justify-items-center">
                  {slide.icons.map((item) => (
                    <div key={item.text} className="flex items-center md:justify-end justify-center gap-2 text-white">
